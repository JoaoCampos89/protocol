    ...
    SourceSamplerBase<ERC20BridgeSamplerContract, ERC20BridgeSamplerContract>
{
    ...
    public async getSellQuotesAsync(
    ...
    ): Promise<DexSample<BalancerFillData>[][]> {
        ...
        const samplesPerPool = await Promise.all(
            pools.map(async poolAddress => this._sellContractHelper.ethCallAsync(
                this._sellContract.sampleSellsFromBalancer,
                [
                    poolAddress,
                    takerToken,
                    makerToken,
    ...
    public async getBuyQuotesAsync(
        ...
        const pools = this._cache.getCachedPoolAddressesForPair(takerToken, makerToken) || [];
        const samplesPerPool = await Promise.all(
            pools.map(async poolAddress => this._buyContractHelper.ethCallAsync(
                this._buyContract.sampleBuysFromBalancer,
                [
                    poolAddress,
                    takerToken,
                    makerToken,
